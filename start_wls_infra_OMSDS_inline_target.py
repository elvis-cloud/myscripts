...
class WebLogicClient:
    ...
    # --- Add OMSDS target within the SAME edit session (no extra locks) ---
    if attach_ds_name:
        try:
            cd('/')
            ds = cmo.lookupJDBCSystemResource(attach_ds_name)
            if ds:
                cd('/JDBCSystemResources/%s' % attach_ds_name)
                _targets = list(get('Targets') or [])
                _me = ObjectName('com.bea:Name=%s,Type=Server' % name)
                if _me not in _targets:
                    _targets.append(_me)
                    set('Targets', jarray.array(_targets, ObjectName))
                    LOG.info('Targeted JDBC DS %s to server %s' % (attach_ds_name, name))
                else:
                    LOG.info('JDBC DS %s already targeted to %s' % (attach_ds_name, name))
            else:
                LOG.warning('JDBC Data Source %s not found; skipping target attach' % attach_ds_name)
        except:
            LOG.error('Failed targeting DS %s to server %s (continuing)' % (attach_ds_name, name))
    # ----------------------------------------------------------------------
...
class CLI:
    ...
    attach_ds_name=("OMSDS" if self.domain.name == "app" else None)
    ),
