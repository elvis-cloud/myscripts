...
class WebLogicClient:
    ...
    # --- Set Self-Tuning Thread Maximum Pool Size to 500 (per server) ---
    try:
        cd('/Servers/%s/ThreadPool/%s' % (name, name))
        set('MaxThreadsConstraint', 500)
        LOG.info('Set Self-Tuning Thread Maximum Pool Size to 500 for server %s' % name)
    except:
        LOG.error('Failed to set Self-Tuning Thread Maximum Pool Size for server %s (continuing)' % name)
    # --------------------------------------------------------------------
