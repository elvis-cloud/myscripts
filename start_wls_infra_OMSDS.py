...
# ---------------------------------------------------------------------
# Ensure JDBC Data Source "OMSDS" is targeted to THIS new server
# Notes:
# - We only do this for the APP domain (not JMS), as requested.
# - This is idempotent: if already targeted, it will no-op.
# - Runs immediately after the managed server MBean is created, so the
#   target assignment is available before (or right after) server start.
# ---------------------------------------------------------------------
if self.domain.name == "app":
    try:
        edit(); startEdit()
        ds_name = "OMSDS"  # <-- Data Source name provided by user
        cd("/")
        ds = cmo.lookupJDBCSystemResource(ds_name)
        if ds is None:
            # Keep going without failing the provisioning. You can re-run
            # this script once the DS exists to attach targets.
            cancelEdit("y")
            LOG.warning("JDBC Data Source %s not found; skipping target attach" % ds_name)
        else:
            serverMBean = cmo.lookupServer(server_name)
            cd("/JDBCSystemResources/%s" % ds_name)
            targets = list(get("Targets") or [])
            me = ObjectName("com.bea:Name=%s,Type=Server" % server_name)
            if me in targets:
                # Nothing to do; keep edit session clean
                cancelEdit("y")
        ...
        try:
            ...
            pass
        LOG.error("Failed to target JDBC DS to new server %s" % server_name)
# ---------------------------------------------------------------------
