...
class WebLogicClient:
    ...
    try:
        ds_name = getattr(self, "_attach_ds_name", None)
        if ds_name:
            cd("/")
            ...
            if ds:
                ...
                if _me not in _targets:
                    ...
                    LOG.info("Targeted JDBC DS %s to server %s" % (ds_name, name))
                else:
                    LOG.info("JDBC DS %s already targeted to %s" % (ds_name, name))
            else:
                LOG.warning("JDBC Data Source %s not found; skipping target attach" % ds_name)
    except:
        LOG.error("Failed targeting DS to server %s (continuing)" % name)
    # ----------------------------------------------------------------------
...
class CLI:
    ...
    def create(self, server_name, ip, app, https_keepalive):
        ...
        self.weblogic.create_machine(server_name, ip)
                # Tell WebLogicClient to attach OMSDS only for app domain
        if self.domain.name == "app":
            self.weblogic._attach_ds_name = "OMSDS"
        else:
            self.weblogic._attach_ds_name = None
